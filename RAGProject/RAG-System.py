...
class Chunk(Base):
    ...
    extra_metadata = Column(JSONB, default={})  # 修正：exrta_metadata -> extra_metadata
    created_at = Column(DateTime, default=datetime.now)
...
class PgVectorRAG:
    ...
    def get_embedding(self, text: str) -> List[float]:
        ...
        if self.config.use_local_embedding and self.embedding_model:
            ...
            # 如果配置使用API但未实现，抛出异常
            raise NotImplementedError("DeepSeek 不提供 Embedding API，请使用本地模型")
...
def main():
    ...
    # 3. 准备知识库文档
    documents = load_documents("knowledge_base/knowledge-digital-products.json")
